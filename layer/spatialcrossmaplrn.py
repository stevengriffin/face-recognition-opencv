...
class SpatialCrossMapLRN(nn.Module):
    ...
    def forward(self, x):
        ...
        for c in range(pad_crop):
            current_sum = current_sum + layer_square.select(1, c).unsqueeze(1)
        ...
        for c in range(1, channels):
            ...
            if c < channels - pad + 1:
                index_next = c + pad - 1
                current_sum = current_sum + layer_square.select(1, index_next).unsqueeze(1)
            # remove
            if c > pad:
                index_prev = c - pad
                current_sum = current_sum - layer_square.select(1, index_prev).unsqueeze(1)
            list_sum.append(current_sum)

...
/root/data/Converter/OpenFace/model =    nn.Sequential( # Sequential,
       ...
       nn.Sequential( # Sequential,
           LambdaReduce(lambda x, y, dim=1: concat_with_pad((x, y), dim),    # DepthConcat,
               ...
               nn.Sequential( # Sequential,
                   nn.MaxPool2d((3, 3), (2, 2)),
                   nn.Conv2d(896, 128, (1, 1), (1, 1)),
                   nn.BatchNorm2d(128),
                   nn.ReLU(),
               ),
               nn.Sequential( # Sequential,
                   nn.Conv2d(896, 384, (1, 1), (1, 1)),
                   nn.BatchNorm2d(384),
                   nn.ReLU(),
               ),
           ),
       ...
       Lambda(lambda x: x.view(-1, 896)),    # View,
       Lambda(lambda x: x.view(x.size(0), -1)), # View,
       nn.Sequential(Lambda(lambda x: x.view(1, -1) if 1==len(x.size()) else x ), nn.Linear(896, 128)), # Linear,
